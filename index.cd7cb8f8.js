...
})({"eDYzt":[function(require,module,exports) {
...
module.bundle.HMR_BUNDLE_ID = "6c5e7f2dcd7cb8f8";
"use strict";
...
},{}],"iJIvz":[function(require,module,exports) {
...
async function update_apps() {
    z2_apps = (await (0, _common.godo)("pm list packages", false, true)).stdout.replaceAll("package:", "").split("\n");
    console.log(z2_apps);
}
...
async function installer(btn) {
    ...
    for (const dep of dependencies){
        console.log(dep);
        if (!z2_apps.includes(dep)) await installer(document.getElementById("install_" + dep));
    }
    ...
    try {
        await (0, _common.install_app)(blobby);
    } catch (e) {
        console.error(e);
        btn.textContent = "Error installing :(";
        return;
    }
    if (btn.getAttribute("data-postinstall") != "undefined") {
        btn.textContent = "Running post-install...";
        try {
            await (0, _common.godo)(btn.getAttribute("data-postinstall"));
        } catch (e) {
            console.error(e);
            btn.textContent = "Error running post-install :(";
            return;
        }
    }
    if (dependencies.includes("moe.shizuku.privileged.api")) {
        // btn.textContent = "Granting Shizuku permissions...";
        // try {
        //     await godo(`pm grant ${btn.getAttribute("data-id")} moe.shizuku.manager.permission.API_V23`);
        // } catch(e) {
        //     console.error(e);
        //     btn.textContent = "Error granting Shizuku permissions :("
        //     return;
        // }
        btn.textContent = "Attempting to start Shizuku to make your life easier...";
        (0, _common.godo)(`sh /sdcard/Android/data/moe.shizuku.privileged.api/start.sh`);
    }
    update_apps();
...
async function render_marketplace() {
    try {
        ...
        console.error(e);
        MODAL_BODY.textContent = "Unable to communicate with Z2.  Please connect Z2 to use the marketplace.";
        return;
    }
    ...
    for (const app_id of Object.keys(mkt_resp.apps)){
        ...
        install_btn.setAttribute("data-postinstall", app.postinstall);
        install_btn.addEventListener("click", ()=>{
            installer(install_btn);
        });
        install_btn.textContent = !z2_apps.includes(app_id) ? "Install" : "Reinstall/Update";
...
},{"@popperjs/core":"7unqC","./common":"2ASYY","bootstrap":"h36JB"}]},["eDYzt","iJIvz"], "iJIvz", "parcelRequire75f2")

//# sourceMappingURL=index.cd7cb8f8.js.map
